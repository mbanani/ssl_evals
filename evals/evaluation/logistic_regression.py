...
class logreg_linear(torch.nn.Module):
    def __init__(self, feat_dim, num_classes):
        super().__init__()
        self.net = torch.nn.Linear(feat_dim, num_classes, bias=True)
        self.net.weight.data.fill_(0.0)
...
class logreg_mha(torch.nn.Module):
    def __init__(self, feat_dim, num_classes):
        super().__init__()

        _feat_dim = feat_dim // 196
        self.class_token = torch.nn.Parameter(torch.zeros(1, 1, _feat_dim))
        self.mha = torch.nn.MultiheadAttention(_feat_dim, 4, batch_first=True)
        self.net = torch.nn.Linear(_feat_dim, num_classes, bias=True)

    def forward(self, x):
        n = x.shape[0]
        x = x.view(n, 256, 196).permute(0, 2, 1).contiguous()

        # batch_class_token = self.class_token.expand(n, -1, -1)
        batch_class_token = x.mean(dim=1, keepdim=True)

        x = torch.cat([batch_class_token, x], dim=1)
        y, _ = self.mha(batch_class_token, x, x, need_weights=False)
        y = y[:, 0, :]
...
class LogisticRegression:
    ...
    def compute_loss(self, feats, labels):
        ...
        wreg = 0.5 * self.logreg.net.weight.norm(p=2)
